...
const Home = () => {
  ...
  return (
    <section id='home' className='flex md:flex-row flex-col px-6 gap-10 max-w-[100%] m-auto'>
      ...
      <div className='flex flex-col sm:flex-row justify-between sm:items-center items-start w-full top-0 ml-0'>
        ...
        <form onSubmit={handleSubmit} className="flex-col sm:flex-row sm:items-center max-w-[100%] w-[80%] flex justify-between mt-5 h-[60px]">
          <input
            ...
            className="w-full sm:w-auto h-[60px] px-4 py-2 mb-4 sm:mb-0 sm:mr-4 rounded-l-lg border border-gray-300 focus:border-blue-500 focus:outline-none text-black cursor-pointer"
            value={email}
            onChange={(e) => setEmail(e.target.value)}
          />
          <button
            type="submit"
            className="w-full sm:w-auto px-4 py-2 rounded-r-lg bg-blue-800 h-[60px] hover:bg-blue-600 text-white font-bold transition duration-300 ease-in-out cursor-pointer"
          >
            Join the waitlist
          </button>
        </form>
